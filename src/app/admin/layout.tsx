...
const learningNavItems = [
  ...
  {
    label: "Announcements",
    href: "/admin/learning/announcements",
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div className="min-h-screen bg-surface-50 dark:bg-surface-950 text-surface-900 dark:text-white transition-colors duration-300">
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          <p className="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-surface-500">Hiring</p>
          ...
          <div className="pt-4 pb-1">
            <p className="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-surface-500">Learning</p>
          </div>
          {learningNavItems.map((item) => {
            const isActive =
              item.href === "/admin/learning"
                ? pathname === "/admin/learning"
                : pathname.startsWith(item.href);
            return (
              <Link
                key={item.href}
