  ...
  const sortedLeaderboard = Array.from(userMarksMap.entries())
    .map(([userId, data]) => {
      const percentage = totalPlatformMarks > 0 
        ? Math.round((data.absoluteMarks / totalPlatformMarks) * 100) 
        : 0;

      return {
        userId,
        name: data.name,
        totalMarks: percentage, // This represents percentage for UI
        absoluteMarks: data.absoluteMarks, // Raw score for dashboard UI
